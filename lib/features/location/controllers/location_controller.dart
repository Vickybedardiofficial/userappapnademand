...
import 'package:apna_demand/features/location/domain/models/prediction_model.dart';
...
import 'package:apna_demand/features/location/widgets/module_dialog_widget.dart';
...
class LocationController extends GetxController implements GetxService {
  ...
  void _prepareZoneData(AddressModel address, bool fromSignUp, String? route, bool canRoute, bool isDesktop) async {
    ...
    getZone(address.latitude, address.longitude, false).then((response) async {
      if (response.isSuccess) {
        ...
        debugPrint('======== >>>>>>>>> rideZoneModel: ${rideZoneModel.zoneId}');
  ...
  void _saveDataAndFirebaseConfig(AddressModel address, bool fromSignUp, String? route, bool canRoute, bool isDesktop) async {
    ...
    debugPrint('======here calling for zone check and config data: ${address.zoneIds} // ${Get.find<StoreController>().store?.zoneId} // ${Get.currentRoute.startsWith(RouteHelper.store)} // ${Get.find<SplashController>().module?.id} //');
    debugPrint('======here calling for zone check and config data: ${Get.currentRoute.startsWith(RouteHelper.initial)}  // ${Get.find<SplashController>().module?.id} //');

    if(Get.currentRoute.startsWith(RouteHelper.initial) && Get.find<SplashController>().module != null) {
        debugPrint('===aaaaaaaa=${Get.find<SplashController>().moduleList?.length}');
        List<int> moduleIds = [];
        await Get.find<SplashController>().getModules().then((v) async {
          if(v) {
            await Future.delayed(const Duration(milliseconds: 1000), () {
              Get.find<SplashController>().moduleList?.forEach((module) {
                debugPrint('======module info: ${module.id} // ${module.moduleType}');
      ...
      if(!moduleIds.contains(Get.find<SplashController>().module!.id)) {
        debugPrint('-------------Module is not available for this location');
  ...
  Future<void> navigateToLocationScreen(String page, {bool offNamed = false, bool offAll = false, bool canRoute = false}) async {
    ...
    if(!fromHome && AddressHelper.getUserAddressFromSharedPref() != null) {
      ...
      if(ResponsiveHelper.isDesktop(Get.context)) {
        final BuildContext? dialogContext = Get.context;
        if(dialogContext == null) {
          return;
        }
        final view = WidgetsBinding.instance.platformDispatcher.views.first;
        final double logicalHeight = view.physicalSize.height / view.devicePixelRatio;
        showGeneralDialog(context: dialogContext, pageBuilder: (_,_,_) {
          return SizedBox(
            height: logicalHeight * 0.75, width: 300,
            child: PickMapScreen(
