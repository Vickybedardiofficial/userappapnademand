...
import 'package:apna_demand/common/widgets/address_widget.dart';
...
import 'package:apna_demand/features/checkout/domain/models/place_order_body_model.dart';
import 'package:apna_demand/features/address/domain/models/address_model.dart';
import 'package:apna_demand/features/cart/domain/models/cart_model.dart';
import 'package:apna_demand/common/models/config_model.dart';
import 'package:apna_demand/features/location/domain/models/zone_response_model.dart';
...
import 'package:apna_demand/helper/custom_validator.dart';
import 'package:apna_demand/helper/date_converter.dart';
import 'package:apna_demand/helper/price_converter.dart';
...
import 'package:apna_demand/common/widgets/custom_dropdown.dart';
...
import 'package:apna_demand/common/widgets/not_logged_in_screen.dart';
import 'package:apna_demand/features/checkout/widgets/checkout_screen_shimmer_view.dart';
import 'package:apna_demand/features/checkout/widgets/payment_method_bottom_sheet.dart';
import 'package:get/get.dart';
import 'package:apna_demand/features/checkout/widgets/bottom_section.dart';
import 'package:apna_demand/features/checkout/widgets/top_section.dart';
...
class CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: guestCheckoutPermission || AuthHelper.isLoggedIn() ? GetBuilder<CheckoutController>(builder: (checkoutController) {
        ...
        debugPrint('====> COD : $_isCashOnDeliveryActive, Digital : $_isDigitalPaymentActive, Offline : $_isOfflinePaymentActive');
          ...
          if(!_isTaxCalled && ((widget.storeId == null && checkoutController.store != null) || widget.storeId != null)) {
            ...
            Future.delayed(const Duration(milliseconds: 50), () {
              ...
              if((widget.storeId == null && _cartList != null && _cartList!.isNotEmpty && placeOrderBody.storeId != null) || (widget.storeId != null)) {
                debugPrint("=======store id: ${placeOrderBody.storeId}");
      ...
      child: SafeArea(
        child: CustomButton(
          ...
          if(isGuestLogIn && guestContactPersonNameController.text.isEmpty) {
            ...
            debugPrint('=====> checkout controller Address: ${checkoutController.address?.toJson()}');
            debugPrint('=====> Final Address: ${finalAddress?.toJson()}');
  ...
  double _calculateOriginalDeliveryCharge({required Store? store, required AddressModel address, required double? distance, required double? extraCharge, double? surgePrice, String? surgePriceType}) {
    ...
    if(store != null && distance != null && distance != -1 && store.selfDeliverySystem == 1) {
      ...
      debugPrint('Fixed shipping charge perKmCharge, minimumCharge, maximumCharge :: $perKmCharge, $minimumCharge, $maximumCharge');
    }

    if(store != null && distance != null) {
      deliveryCharge = distance * perKmCharge;
      debugPrint('distance * perKmCharge = $deliveryCharge');

      if(deliveryCharge < minimumCharge) {
        debugPrint('minimumCharge $minimumCharge');
        deliveryCharge = minimumCharge;
        debugPrint('minimumCharge 2 $deliveryCharge');
      }else if(maximumCharge != null && deliveryCharge > maximumCharge) {
        debugPrint('maximumCharge $deliveryCharge');
        deliveryCharge = maximumCharge;
        debugPrint('maximumCharge 2 $maximumCharge');
      }
    }
    ...
    if(store != null && store.selfDeliverySystem == 0 && extraCharge != null) {
      ...
      debugPrint('extraCharge $deliveryCharge');
    }
    ...
    if(store != null && store.selfDeliverySystem == 0 && surgePrice != null && surgePrice > 0) {
      if(surgePriceType == 'percent') {
        ...
        debugPrint('badWeatherChargeForToolTip $badWeatherChargeForToolTip');
        deliveryCharge = deliveryCharge + (deliveryCharge * (surgePrice/100));
        debugPrint('(deliveryCharge * (surgePrice/100)) $deliveryCharge');
      } else {
        badWeatherChargeForToolTip = surgePrice;
        debugPrint('badWeatherChargeForToolTip $badWeatherChargeForToolTip 2');
        deliveryCharge = deliveryCharge + surgePrice;
        debugPrint('deliveryCharge + surgePrice $deliveryCharge');
      }
    }
